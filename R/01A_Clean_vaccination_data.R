...
clean.vaccination.data <- function(vaccine, 
                                   country_codes, 
                                   population,
  ...
  England_vaccinations_clean <- England_vaccinations %>% 
    ...
    mutate(ReportCountry = "United Kingdom (England)",
           TargetGroup = ifelse(TargetGroup == "1_Age60+", "Age60+", TargetGroup),
  ...
  Scotland_vaccination_clean <- Scotland_vaccination %>% 
    mutate(TargetGroup = ifelse(TargetGroup %in% c("Age60_69", "Age70_79", "Age80+"), "1_Age60+", TargetGroup)) %>% 
  ...
  # EU/EEA countries
  vaccine_denominators <- population %>% 
  ...
  # Find missing doses for some countries --------------------------------------
  # Reduction of doses
  ...
  change_booster2_age60 <- mean_pc_change_values %>% filter(TargetGroup == "Age 60+") %>% pull(ValueChange3To4)
  change_booster3_age60 <- mean_pc_change_values %>% filter(TargetGroup == "Age 60+") %>% pull(ValueChange4To5)
  ...
  lag_booster2_age60 <- mean_week_difference %>% filter(TargetGroup == "Age 60+" & Dose == "booster 2") %>% pull(MeanWeeks)
  lag_booster3_age60 <- mean_week_difference %>% filter(TargetGroup == "Age 60+" & Dose == "booster 3") %>% pull(MeanWeeks)
  ...
  # Moldova
  MD_booster_2 <- vaccine_data_clean %>% 
    filter(ReportCountry == "Republic of Moldova"  & TargetGroup == "Age 60+") %>% 
    group_by(TargetGroup) %>% 
    filter(DoseAdditional1 > 0) %>%
    ungroup() %>% 
    slice(lag_booster2_age60 : n()) %>% 
    #slice(1 : lag_booster2_age60) %>% 
  ...
  CH_60_B2 <- vaccine_data_clean %>% 
    filter(ReportCountry == "Switzerland"  & TargetGroup == "Age 60+") %>% 
    group_by(TargetGroup) %>% 
    filter(DoseAdditional1 > 10) %>%
    ungroup() %>% 
    slice(lag_booster2_age60 : n()) %>% 
  ...
  CH_booster_2 <- CH_25_B2 %>% full_join(CH_50_B2) %>% full_join(CH_60_B2) 
  
  
  # Ukraine
  UA_booster_2 <- vaccine_data_clean %>% 
    filter(ReportCountry == "Ukraine"  & TargetGroup == "Age 60+") %>% 
    group_by(TargetGroup) %>% 
    filter(DoseAdditional1 > 0) %>%
    ungroup() %>% 
    slice(lag_booster2_age60 : n()) %>% 
  ...
  booster_2_data <- MD_booster_2 %>% full_join(CH_booster_2) %>% full_join(UA_booster_2) %>% full_join(ENG_booster_2)%>% 
  ...
  HR_60_B3 <- temp %>% 
    filter(ReportCountry == "Croatia"  & TargetGroup == "Age 60+") %>% 
    group_by(TargetGroup) %>% 
    filter(DoseAdditional2 > 0) %>%
    ungroup() %>% 
    slice(lag_booster3_age60 : n()) %>% 
    #slice(1 : lag_booster3_age60) %>% 
    mutate(DoseAdditional3 = round(DoseAdditional2 * change_booster3_age60))
  
  HR_booster_3 <- HR_25_B3 %>% full_join(HR_50_B3) %>% full_join(HR_60_B3) 
  
  # Germany
  DE_booster_3 <- temp %>% 
    filter(ReportCountry == "Germany"  & TargetGroup == "Age 60+") %>% 
    group_by(TargetGroup) %>% 
    filter(DoseAdditional2 > 0) %>%
    ungroup() %>% 
    slice(lag_booster3_age60 : n()) %>% 
  ...
  IL_60_B3 <- temp %>% 
    filter(ReportCountry == "Israel"  & TargetGroup == "Age 60+") %>% 
    group_by(TargetGroup) %>% 
    filter(DoseAdditional2 > 0) %>%
    ungroup() %>% 
    slice(lag_booster3_age60 : n()) %>% 
  ...
  IL_booster_3 <- IL_25_B3 %>% full_join(IL_50_B3) %>% full_join(IL_60_B3) 
  ...
  LV_60_B3 <- temp %>% 
    filter(ReportCountry == "Latvia"  & TargetGroup == "Age 60+") %>% 
    group_by(TargetGroup) %>% 
    filter(DoseAdditional2 > 0) %>%
    ungroup() %>% 
    slice(lag_booster3_age60 : n()) %>% 
  ...
  LV_booster_3 <- LV_25_B3 %>% full_join(LV_50_B3) %>% full_join(LV_60_B3) 
  ...
  LT_60_B3 <- temp %>% 
    filter(ReportCountry == "Lithuania"  & TargetGroup == "Age 60+") %>% 
    group_by(TargetGroup) %>% 
    filter(DoseAdditional2 > 0) %>%
    ungroup() %>% 
    slice(lag_booster3_age60 : n()) %>% 
  ...
  LT_booster_3 <- LT_25_B3 %>% full_join(LT_50_B3) %>% full_join(LT_60_B3) 
  ...
  CH_booster_3 <- CH_25_B3 %>% full_join(CH_50_B3) %>% full_join(CH_60_B3) 
  ...
  SE_60_B3 <- temp %>% 
    filter(ReportCountry == "Sweden"  & TargetGroup == "Age 60+") %>% 
    group_by(TargetGroup) %>% 
    filter(DoseAdditional2 > 0) %>%
    ungroup() %>% 
    slice(lag_booster3_age60 : n()) %>% 
  ...
  SE_booster_3 <- SE_25_B3 %>% full_join(SE_50_B3) %>% full_join(SE_60_B3) 
  ...
  SL_60_B3 <- temp %>% 
    filter(ReportCountry == "Slovenia"  & TargetGroup == "Age 60+") %>% 
    group_by(TargetGroup) %>% 
    filter(DoseAdditional2 > 0) %>%
    ungroup() %>% 
    slice(lag_booster3_age60 : n()) %>% 
  ...
  SL_booster_3 <- SL_25_B3 %>% full_join(SL_50_B3) %>% full_join(SL_60_B3) 
  ...
  RO_60_B3 <- temp %>% 
    filter(ReportCountry == "Romania"  & TargetGroup == "Age 60+") %>% 
    group_by(TargetGroup) %>% 
    filter(DoseAdditional2 > 0) %>%
    ungroup() %>% 
    slice(lag_booster3_age60 : n()) %>% 
  ...
  RO_booster_3 <- RO_25_B3 %>% full_join(RO_50_B3) %>% full_join(RO_60_B3) 
  
  
  booster_3_data <- MD_booster_3 %>% 
    full_join(DE_booster_3) %>% 
    full_join(IL_booster_3) %>% 

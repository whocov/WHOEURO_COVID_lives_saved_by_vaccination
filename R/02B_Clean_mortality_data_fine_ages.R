...
# Version1: October 2023


calculate.mortality.by.finer.age.groups.update <- function(deaths, 
                                                           Croatia_mortality,
                                                           Iceland_mortality, 
                                                           Israel_mortality, 
                                                           Kosovo_mortality, 
  ...
  # Iceland
  Iceland_mortality_clean <- clean.Iceland.mortality.data(Iceland_mortality, resolution = "finer ages")
  ...
  Romania_mortality_clean <- Romania_mortality %>% 
    ...
    # melt data frame   
    group_by(CountryName, Period) %>% 
    pivot_longer(cols = c("Deaths20-24F", "Deaths20-24M", "Deaths25-29F", "Deaths25-29M", "Deaths30-39F", "Deaths30-39M", 
                          "Deaths40-49F", "Deaths40-49M", "Deaths50-59M", "Deaths50-59F", "Deaths60-64M", "Deaths60-64F", 
                          "Deaths65-69M", "Deaths65-69F", "Deaths70-74M", "Deaths70-74F", "Deaths75-79M", "Deaths75-79F",
                          "Deaths80+M", "Deaths80+F"),
                 names_to = "age_group",
  ...
  deaths_age <- deaths_age %>%
    mutate(age_group = ifelse(age_group %in% c("Age20-24", "Age25-29", "Age30-39", "Age40-49"), "Age25-49", age_group),
           age_group = ifelse(age_group %in% c("Age60-64", "Age65-69"), "Age60-69", age_group),
           age_group = ifelse(age_group %in% c("Age70-74", "Age75-79"), "Age70-79", age_group),
           age_group = ifelse(age_group %in% c("Age80+-"), "Age80+", age_group)) %>% 
  ...
  deaths_age <- deaths_age %>% 
    filter(!(CountryName %in% c("Germany", "Republic of Moldova", "Ukraine") & 
               age_group %in% c("Age25-49", "Age50-59", "Age60-69", "Age70-79", "Age80+"))) %>% 
    filter(!(CountryName == "Israel" & age_group %in% c("Age60-69", "Age70-79", "Age80+"))) %>% 
    ...
    mutate(TargetGroup = ifelse(age_group == "Age25-49", "Age 25-49",
                                ifelse(age_group == "Age50-59", "Age 50-59", 
                                       ifelse(age_group == "Age60-69", "Age 60-69",
                                              ifelse(age_group == "Age70-79", "Age 70-79", 
                                                     ifelse(age_group == "Age80+", "Age 80+", age_group)))))) %>% 
  ...
  full_template <- expand.grid(ReportCountry = unique(deaths_age$ReportCountry),
    ...
    distinct() %>% 
    filter(!TargetGroup == "Age 60+") %>% 
    filter(!(ReportCountry %in% c("Germany", "Republic of Moldova", "Ukraine") & 
               TargetGroup %in% c("Age 25-49", "Age 50-59", "Age 60-69", "Age 70-79", "Age 80+"))) %>% 
    filter(!(ReportCountry %in% c("Israel", "United Kingdom (Scotland)") & 
               TargetGroup %in% c("Age 60-69", "Age 70-79", "Age 80+")))

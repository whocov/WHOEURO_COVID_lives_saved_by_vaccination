...
create.summary.panel.plots.finer.older <- function(Country_population_finer, 
  ...
  Country_population_60 <- Country_population_finer %>% 
    filter(TargetGroup %in% c("Age 60-69", "Age 70-79", "Age 80+")) %>% 
    group_by(Year, CountryName) %>% 
    summarise(Denominator = sum(Denominator)) %>% 
    ungroup() %>% 
    add_column(TargetGroup = "Age 60+", .before = "Denominator")
  
  Country_population_complete <- full_join(Country_population_finer, Country_population_60)
  ...
  rates_cases_age <- cases %>%
    ...
    summarise(`Age_25-49` = sum(c(Age25.29F, Age25.29M, Age30.39F, Age30.39M, Age40.49F, Age40.49M), na.rm = TRUE),
              ...
              `Age_60+` = sum(c(Age60.64F, Age60.64M, Age65.69F, Age65.69M,
                                Age70.74F, Age70.74M, Age75.79F, Age75.79M,
                                Age80.F, Age80.M), na.rm = TRUE),
              `Age_60-69` = sum(c(Age60.64F, Age60.64M, Age65.69F, Age65.69M), na.rm = TRUE),
              `Age_70-79` = sum(c(Age70.74F, Age70.74M, Age75.79F, Age75.79M), na.rm = TRUE),
              `Age_80+` = sum(c(Age80.F, Age80.M), na.rm = TRUE)) %>%
    ungroup() %>%
    # melt data frame    
    pivot_longer(cols = `Age_25-49`:`Age_80+`,
    ...
    full_join(Country_population_finer, 
  ...
  mortality_data_plot_temp1 <- deaths_age_finer %>% 
    ungroup() %>% 
  ...
  mortality_data_plot_temp2 <- deaths_age %>% 
    filter(ReportCountry %in% c("Germany", "Israel", "Republic of Moldova", "United Kingdom (Scotland)", "Ukraine"))
  
  
  mortality_data_plot <- full_join(mortality_data_plot_temp1, mortality_data_plot_temp2) %>% 
    ...
    full_join(Country_population_complete, 
  ...
  row.2A <- ggplot() +
    geom_segment(data = variant_waves_region, aes(x = start, xend = end, y = 80, yend = 80)) +
    geom_segment(data = variant_waves_region, aes(x = start, xend = start, y = 78, yend = 82)) +
    geom_text(data = variant_plot_timings, aes(x = Position, y = 90, label = Variant), size = 4) +
    geom_line(data = mortality_data_plot, aes(x = year_week, y = Observed_MR, group = 1, linetype = "Observed_MR"), size = 1) +
    facet_wrap(.~ TargetGroup, nrow = 1) +
    scale_linetype_manual(values = c("Observed_MR" = "solid"),
                          labels = c("Observed_MR" = "Reported\nmortality"),
                          name = "") +
    ...
    labs(subtitle = "B) Reported mortality rates",
         x = "Week",
         y = "Reported mortality rate")
  ...
  row.2B <- ggplot() +
    geom_segment(data = variant_waves_region, aes(x = start, xend = end, y = 2850, yend = 2850)) +
    geom_segment(data = variant_waves_region, aes(x = start, xend = start, y = 2840, yend = 2860)) +
    geom_text(data = variant_plot_timings, aes(x = Position, y = 4500, label = Variant), size = 4) +
    ...
    guides(colour = guide_legend(nrow = 2),
           fill = guide_legend(nrow = 2)) +
   # annotation_logticks() +
    labs(subtitle = "A) Reported infection and mortality rates",
         x = "Week",
         y = "Reported rates (log 10)")
  ...
  # Calculate vaccination rate for 60+ age group -------------------------------
  vaccination_data_plot_60_long <- vax_clean %>%
    ...
    summarise(FirstDose = sum(FirstDose),
              ...
              Denominator = sum(unique(Denominator))) %>% 
    ungroup() %>% 
    add_column(TargetGroup = "Age 60+", .before = "FirstDose")
  ...
  vaccination_data_plot_total <- vax_clean %>%
    ...
    summarise(FirstDose = sum(FirstDose),
              ...
              Denominator = sum(unique(Denominator))) %>% 
    ungroup() %>% 
    full_join(vaccination_data_plot_60_long) %>% 
    group_by(TargetGroup, year_week) %>%
    summarise(FirstDose = sum(FirstDose),
              SecondDose = sum(SecondDose),
              DoseAdditional1 = sum(DoseAdditional1),
    ...
    mutate(Dosage = factor(Dosage, levels = legend.order),
           Rate = (Rate / 1000),
           TargetGroup = ifelse(TargetGroup == "Age 25-49", "25 to 49 years",
                                ifelse(TargetGroup == "Age 50-59", "50 to 59 years",
                                       ifelse(TargetGroup == "Age 60-69", "60 to 69 years",
                                              ifelse(TargetGroup == "Age 70-79", "70 to 79 years",
                                                     ifelse(TargetGroup == "Age 80+", "≥80 years", "≥60 years"))))),
  ...
  row.3A <- ggplot() +
    geom_segment(data = variant_waves_region, aes(x = start, xend = end, y = 12, yend = 12)) +
    geom_segment(data = variant_waves_region, aes(x = start, xend = start, y = 11.5, yend = 12.5)) +
    geom_text(data = variant_plot_timings, aes(x = Position, y = 13, label = Variant), size = 4) +
    geom_line(data = vaccination_data_plot_total,
              aes(x = year_week, y = Rate, group = Dosage, colour = Dosage), size = 1) +
    facet_wrap(.~ TargetGroup, nrow = 1) +
  ...
  vaccination_data_plot_cumul_60 <- vax_clean %>%
    filter(ReportCountry %in% unique(countries_included$CountryName)) %>% 
    filter(TargetGroup %in% c("Age 60-69", "Age 70-79", "Age 80+")) %>% 
    arrange(ReportCountry, TargetGroup, year_week) %>% 
    group_by(year_week) %>%
  ...
  vaccination_data_plot_cumul <- vax_clean %>%
    ...
    summarise(FirstDose = sum(FirstDose),
              ...
              denominator = sum(unique(Denominator))) %>% 
    ungroup() %>% 
    full_join(vaccination_data_plot_cumul_60) %>% 
    group_by(TargetGroup, year_week) %>%
    summarise(FirstDose = sum(FirstDose),
              SecondDose = sum(SecondDose),
              DoseAdditional1 = sum(DoseAdditional1),
  ...
  row.3B <- ggplot() +
    ...
    geom_text(data = variant_plot_timings, aes(x = Position, y = 107, label = Variant), size = 4) +
  ...
  expected_plot_60 <- Expected_cases_all_ages %>%
    ungroup() %>% 
    select(ReportCountry, year_week, TargetGroup, DeathsObserved, DeathsExpected, Denominator) %>%
  ...
  row.4 <- ggplot() +
    geom_segment(data = variant_waves_region, aes(x = start, xend = end, y = 125, yend = 125)) +
    geom_segment(data = variant_waves_region, aes(x = start, xend = start, y = 122, yend = 127)) +
    geom_text(data = variant_plot_timings, aes(x = Position, y = 135, label = Variant), size = 4) +
    geom_line(data = expected_plot_total,
              aes(x = year_week, y = Rates, group = Mortality, colour = Mortality), size = 1) +
    facet_wrap(.~ TargetGroup, nrow = 1) +
    scale_color_manual(name = "Mortality rates",
                       values = c("Expected_MR" = "#D6852B", 
                                  "Observed_MR" = "#003C50"),
                       labels = c("Expected_MR" = "Expected", 
                                  "Observed_MR" = "Reported")) +
  ...
  expected_plot_cum_60 <- Expected_cases_all_ages %>%
    ungroup() %>% 
    select(ReportCountry, year_week, TargetGroup, DeathsObserved, DeathsExpected, Denominator) %>%
    filter(TargetGroup %in% c("Age 60-69", "Age 70-79", "Age 80+", "Age 60+")) %>% 
    mutate(TargetGroup = "Age 60+") %>% 
  ...
  expected_plot_cum <- Expected_cases_all_ages %>%
    ...
    ungroup() %>% 
    filter(!TargetGroup == "Age 60+") %>% 
    full_join(expected_plot_cum_60) %>% 
    arrange(TargetGroup, year_week) %>% 
    group_by(TargetGroup) %>% 
    mutate(DeathsObservedCum = cumsum(DeathsObserved),
           DeathsExpectedCum = cumsum(DeathsExpected),
           Denominator = sum(unique(Denominator))) %>% 
    ungroup()  %>% 
  ...
  row.5 <- ggplot() +
    geom_segment(data = variant_waves_region, aes(x = start, xend = end, y = 890, yend = 890)) +
    geom_segment(data = variant_waves_region, aes(x = start, xend = start, y = 885, yend = 895)) +
    geom_text(data = variant_plot_timings, aes(x = Position, y = 915, label = Variant), size = 4) +
  ...
  summary_panel_final <- (row.2B / row.3A / row.3B / row.4 / row.5) + plot_layout(ncol = 1)

  if(to.save == TRUE){
    ggsave("Outputs/WHO_EURO_Lives_saved_COVID_vaccines_Figure_1.png", dpi=300, height = 12, width = 14)
  }
  
  return(summary_panel_final)

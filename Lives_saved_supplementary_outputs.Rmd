...
```{r, warning = FALSE, message = FALSE, comment = FALSE, echo = FALSE}
  ...
  Supp.Table.1 <- flextable(countries_included_summary) %>%
     theme_vanilla() %>%
     ...
     add_footer_row(values = Supp.table.1.footnote, colwidths = 6) %>%
     set_table_properties(layout = "autofit") %>% 
     fontsize(size = 9, part = "footer")  
  
  Supp.Table.1
  #save_as_docx(Supp.Table.1, path = "Outputs/Deaths_averted_supp_table_1.docx")


```

\newpage
Supplementary Table 2: Age group and country breakdown of vaccination uptake (VU), number of deaths observed and averted as well as observed and expected Mortality Rates (MR), up to week 12/2023 in population aged 25 years and over, for the 31 European Region countries with sufficient data.
```{r, warning = FALSE, message = FALSE, comment = FALSE, echo = FALSE}

Supp.table.2.footnote <- paste0("Mortality rates are per 100,000 population.\n", Kosovo.footnote)


  ST2 <- create.table.4(Expected_cases_all_ages, method = "by age older finer") %>% 
    mutate(TargetGroup = str_replace(TargetGroup, "^Age ", "")) %>% 
    filter(!TargetGroup == "TotalDoses")
  
  Supp.Table.2 <- flextable(ST2) %>% 
     theme_vanilla() %>%
...
Supp.table.3.footnote <- paste0("Mortality rates are per 100,000 population.\n", Kosovo.footnote)
  ...
  Supp.table.3 <- create.table.4(full.time.data, method = "by age older finer variant") %>% 
  ...
  Supp.table.3.pre.omicron <- create.table.4(pre.omicron.data, method = "by age older finer variant") %>% 
    filter(ReportCountry %in% countries_partial_reporting)
  

  
  temp.supp.table.3 <- full_join(Supp.table.3, Supp.table.3.pre.omicron) %>% 
     arrange(ReportCountry) %>% 
     mutate(TargetGroup = str_remove(TargetGroup, "Age "))
  
Supp.Table.3 <- flextable(temp.supp.table.3) %>%
   theme_vanilla() %>%
   ...
   fontsize(size = 10, part = "body") %>%
   merge_v(j = "ReportCountry", target = "ReportCountry") %>% 
   add_footer_row(values = Supp.table.3.footnote, colwidths = 17) %>%
   set_table_properties(layout = "autofit") %>% 
   fontsize(size = 9, part = "footer")  
...
\newpage
Supplementary Table 5. Regional summary of sensitivity analyses by age and variant, for all `r n_countries` countries, areas and territories (CAT) included between weeks 50/2020 and 12/2023.
```{r, warning = FALSE, message = FALSE, comment = FALSE, echo = FALSE}

supp.table.5.footnote <- "Scenario descriptions: 1: High VE values; 2: Low VE values; 3: Long lag time; 4: Short lag time; 5: Long vaccine waning time; 6: Short vaccine waning time; 7: High prior immunity; 8: Low prior immunity"

st5 <- create_sensitivity_analyses_summary_table(countries_included, resolution = "Region") %>% 
  mutate(TargetGroup = str_remove(TargetGroup, "Age "),
...
Supp.Table.5 <- flextable(st5) %>%
  ...
  align(align = "center", part = "all") %>% 
  add_footer_row(values = supp.table.5.footnote, colwidths = 17) %>% 
  set_table_properties(layout = "autofit") 
...
\newpage
Supplementary table 6. Results of sensitivity analyses according to each alternative Vaccine Effectiveness (VE) scenario, showing the total number of deaths averted for each countries, areas and territories (CAT), expected mortality rate per 100,000 population and the % expected deaths averted by vaccination for the population aged 25 years and older, in `r n_countries` European Region CAT, for weeks 51/2020 to 12/2023.
```{r, warning = FALSE, message = FALSE, comment = FALSE, echo = FALSE}


st6 <- create_sensitivity_analyses_summary_table(countries_included, resolution = "Country") %>% 
...
Supp.Table.6 <- flextable(st6) %>%
  ...
  merge_v(j = "ReportCountry", target = "ReportCountry") %>% 
  add_footer_row(values = supp.table.5.footnote, colwidths = 7) %>% 
